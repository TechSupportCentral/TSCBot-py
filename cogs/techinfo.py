...
class techinfo(commands.Cog):
    ...
    async def linuxusb(self, ctx):
        ...
        embed.add_field(name="Step 1:", value="Decide what works best for you.\nFor those new to computers, we recommend [Pop!_OS](https://system76.com/cosmic/) as a user-friendly and accessible option.\nIf you're experienced with computers but not with Linux, choose a desktop environment:\n- If you prefer [KDE Plasma](https://kde.org/), use [Fedora KDE](https://fedoraproject.org/spins/kde)\n- If you prefer [GNOME](https://www.gnome.org/), use [Fedora Workstation](https://fedoraproject.org/workstation/)\n- If you prefer [Cosmic](https://system76.com/cosmic/), use [Pop!_OS](https://system76.com/cosmic/)", inline=False)
        embed.add_field(name="Step 2:", value="Download the Linux ISO from one of the links above. If you chose Fedora, make sure to download the x86_64 ISO instead of Fedora Media Writer.\nFor the flashing tool, you can use [balenaEtcher](https://balena.io/etcher/) or [Rufus](https://rufus.ie). balenaEtcher is simple and gets the job done fast; Rufus has many more features, but it can be confusing to beginners.", inline=False)
        ...
        embed.add_field(name="Step 4:", value="Boot into Linux.\nTurn on the computer and start spamming the boot menu key (figure out which one yours is with the `/bootkeys` command) until the menu appears. When the menu shows up, select the USB to boot to.", inline=False)
