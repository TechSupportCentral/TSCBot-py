...
class techinfo(commands.Cog):
    ...
    async def driver_managers(self, ctx):
        ...
        embed.add_field(name="Graphics Drivers", value="Even if your computer is a prebuilt, you should still get your graphics drivers directly from the manufacturer of the GPU ([AMD](https://amd.com/en/support), [NVIDIA](https://nvidia.com/Download/index.aspx), [Intel](https://www.intel.com/content/www/us/en/support/detect.html), etc).", inline=False)
    ...
    async def windowsusb(self, ctx):
        ...
        embed.add_field(name="Step 3:", value="Open Rufus. Next to the Select button, there should be an arrow. If not, try enabling [checking for updates](https://github.com/pbatard/rufus/wiki/FAQ#help-i-dont-see-the-iso-download-button).\nClick the arrow, then choose download. Press the download button. Select Windows 11 in your preferred edition, and choose English International. Specify where Rufus should save the ISO, and it will begin downloading.", inline=False)
        embed.add_field(name="Step 4:", value="When Rufus is done downloading the ISO, select your USB flash drive in the device menu. When you flash Windows to the drive, its previous contents will be erased, so now would be a good time to back them up if needed.\nSelect GPT as the partition scheme. Rufus should now look like [this](https://i.imgur.com/pgKRiJM.png). If so, click 'Start' and then 'OK' to begin the flashing process.\nWhen prompted, enable:\n- Remove requirement for 4GB+ RAM, Secure Boot and TPM 2.0\n- Remove requirement for an online Microsoft account\n- Disable data collection (Skip privacy questions)", inline=False)
        embed.add_field(name="Step 5:", value="After the flashing process is complete, turn off the computer you want to install Windows on, and insert the USB if you haven't already.\nTurn it back on and start spamming the boot menu key (figure out which one yours is with the `/bootkeys` command) until the menu appears. When the menu shows up, select the USB to boot to.", inline=False)
        ...
        embed.add_field(name="Step 8:", value="When you get to the next stage of setup, make sure to use a local account and avoid all prompts encouraging you to switch to a Microsoft one. When asked to sign-in, click 'I don't have internet' or 'Domain join instead'.\n**Note:** If you find yourself unable to do so, press Shift + F10 to open the command prompt. Type in `OOBE\BYPASSNRO` and press enter.", inline=False)
        ...
        embed.add_field(name="Step 10:", value="Format your USB so you can use it again.\nOpen File Explorer and find your USB flash drive. Right click it and select format. Use NTFS as the filesystem and tick the Quick Format box. Once the format is complete, your drive will be empty and you can use it for your own purposes again.", inline=False)
        await ctx.send(embed=embed)
    ...
    async def linuxusb(self, ctx):
        ...
        embed.add_field(name="Step 1:", value="Decide what works best for you.\nFor those new to computers, we highly recommend [Pop!_OS](https://system76.com/cosmic/) as a user-friendly and accessible option.\nIf you're experienced with computers but not with Linux, choose a desktop environment:\n- If you prefer [KDE Plasma](https://kde.org/), we suggest [Fedora KDE](https://fedoraproject.org/spins/kde)\n- If you prefer [GNOME](https://www.gnome.org/), we suggest [Fedora Workstation](https://fedoraproject.org/workstation/)\n- If you prefer [Cosmic](https://system76.com/cosmic/), we suggest [Pop!_OS](https://system76.com/cosmic/)", inline=False)
        embed.add_field(name="Step 2:", value="Download the Linux ISO from one of the links above. If you chose Fedora, make sure to download the x86_64 ISO instead of Fedora Media Writer.", inline=False)
        embed.add_field(name="Step 3:", value="Download [Rufus](https://github.com/pbatard/rufus/releases/latest), the tool you will use to download and flash Linux to the USB.", inline=False)
        embed.add_field(name="Step 4:", value="Once everything is downloaded, use Rufus to flash your ISO to a USB (must be 4 GB or larger).\nSelect the Linux ISO you downloaded and the USB you want to flash to, click 'Start' and then 'OK' to begin the flashing process.", inline=False)
        embed.add_field(name="Step 5:", value="After the flashing process is complete, turn off the computer you want to install Linux on, and insert the USB if you haven't already.\nTurn it back on and start spamming the boot menu key (figure out which one yours is with the `/bootkeys` command) until the menu appears. When the menu shows up, select the USB to boot to.", inline=False)
        embed.add_field(name="Step 6:", value="Your computer will boot to show the Linux desktop. Note that Linux is not installed on your computer; this is called a Live USB, where you can try it out before installation. While anything you do on the USB in this state will not be saved, be aware that certain actions, such as modifying partitions, can have permanent and irreversible effects on your PC. If you wish to replace Windows with Linux, open the 'Install Linux' file on the desktop. If you want to return to Windows on the next reboot, simply restart the PC with the USB unplugged.", inline=False)
        await ctx.send(embed=embed)
    ...
    @commands.hybrid_command(description="Get instructions on how to run the system file checker in Windows")
    async def windowsrepair(self, ctx):
        embed = discord.Embed(title="How to repair Windows", description="System File Checker is a utility in Windows that checks for problems with files on your computer.", color=0x00a0a0)
    ...
    async def legacywindows(self, ctx):
        ...
        embed.add_field(name="What can I do?", value="If your computer is good enough to run Windows 11 reliably, download the [Windows 11 Installation Assistant](https://www.microsoft.com/en-us/software-download/windows11).\nIf your computer isn't good enough to run Windows 11 very well, it's recommended to switch to a lighter Linux distribution such as [Debian](https://www.debian.org/) with [LXQt](https://lxqt-project.org/). Instructions are available using the `/linuxusb` command.", inline=False)
        await ctx.send(embed=embed)
